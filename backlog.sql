...
CREATE TABLE IF NOT EXISTS `xref_purchase_dlc` (
  `purchase_id` int(11) NOT NULL,
  `dlc_id` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
ALTER TABLE `xref_menu_library`
 ADD UNIQUE KEY `menu_id_2` (`menu_id`,`library_id`), ADD KEY `library_id` (`library_id`), ADD KEY `menu_id` (`menu_id`);

ALTER TABLE `xref_purchase_dlc`
 ADD KEY `purchase_id` (`purchase_id`), ADD KEY `dlc_id` (`dlc_id`);
...
ALTER TABLE `xref_purchase_dlc`
ADD CONSTRAINT `xref_purchase_dlc_ibfk_1` FOREIGN KEY (`purchase_id`) REFERENCES `purchase` (`purchase_id`),
ADD CONSTRAINT `xref_purchase_dlc_ibfk_2` FOREIGN KEY (`dlc_id`) REFERENCES `dlc` (`dlc_id`);
