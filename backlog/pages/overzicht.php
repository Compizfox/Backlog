...
$script = "<script>var ctx = document.getElementById(\"linechart\").getContext(\"2d\"); var data = $json; var myLineChart = new Chart(ctx).Line(data);</script>";

// Monthly linechart
$query = "SELECT DATE_FORMAT(date, '%Y-%m') AS date, SUM(count) AS count FROM (SELECT date, COUNT(*) AS count FROM history JOIN status ON status_id = new_status WHERE date != 0000-00-00 AND completed = 1 GROUP BY date) AS a GROUP BY DATE_FORMAT(date, '%Y%m')";
$result = $mysqli->query($query) or die($query);
